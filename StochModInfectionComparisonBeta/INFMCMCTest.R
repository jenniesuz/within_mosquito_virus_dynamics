...
llikePrior <- function(fit.params="infRate", ## parameters to fit
                       ref.params = virus_params(), ## reference parameters
                       obsDat=testDat) { ## observed data
  ...
  nll.binom(parms, dat=obsDat) + lprior(parms)
}

llikePrior(obsDat=testDat)
...
logParms(c(infRate = 10^-6, muV=0.1))
...
unlogParms(logParms(c(infRate = 10^-6, muV=0.1)))

## set bounds on initial parameter guesses
initBounds <- data.frame(rbind( ## for initial conditions
  c(log(10^-8),log(10^-6.5)
    ,c(log(0.05),log(0.15))) ## infRate
)) ## 

colnames(initBounds) <- c('lower','upper')
rownames(initBounds) <- c('loginfRate','logmuV')
...
samp_Seq <- mcmcSampler(init.params = c(infRate=10^-7,muV=0.1)
                        , seed = 1
